...
export default function CardComponent({ card, selected, onClick, small, faceDown, groupLabel }: Props) {
  if (faceDown) {
    return (
      <div
        className={`
  ...
  return (
    <div
      ...
      className={`
        ...
        ${selected ? 'border-yellow-400 -translate-y-3 shadow-yellow-400/50' : 'border-gray-200 hover:-translate-y-1'}
        ${card.isJoker ? 'border-purple-400' : ''}
      `}
    >
      {groupLabel && (
